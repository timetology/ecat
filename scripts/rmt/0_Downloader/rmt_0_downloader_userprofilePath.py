...
# 
# Description: Crawl NetWitness Endpoint looking for possible locations of %userprofile% per system and then download the inputed filepath.
#
# Example
#
# Change Log
# .1	25Mar2019 - Initial Development
...
OR pa.path like N'%:\Documents and Settings\%')
...
#				dirlist.append(s[0] + '\\' + s[1] + '\\' + s[2] + '\\NTUSER.DAT')
			...
			for i in dirlist:
				...
				try:
					...
					sleep(.1)
def main():
	parser = argparse.ArgumentParser(description='Download from %userprofile%')
  ...
  # Add escape character to filepaths
  if (args.filepath):
