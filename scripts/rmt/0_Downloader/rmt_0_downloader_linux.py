...
def main():
	...
	#Get Machine List (OSType = 1 is Windows, 2 = Mac, 3 = Linux)
	txt = "select PK_Machines from dbo.Machines where dbo.Machines.FK_OSTypes = 3 AND dbo.Machines.MarkedAsDeleted = 0"
	...
	hosts = "select MachineName from dbo.Machines where dbo.Machines.FK_OSTypes = 3 and dbo.Machines.MarkedAsDeleted = 0"
	...
	if g_debug:
		...
		for list in tqdm(lol(rows,int(args.batch),int(current_hosts))):
			...
			for artifact in artifactList:
				...
				sql = sql + "insert into @p5 values(N'Path',N'" + artifact + "')\n"
				sql = sql + "insert into @p5 values(N'maxfiles',N'" + args.maxfiles + "')\n"
				sql = sql + "exec dbo.usp_InsertMachineCommand @MachinesPKList=@p1,@Type=256,@Comment=N'Download RSA rmt_0_downloader_linux',@IsAutomatic=0,@CommandParams=@p5"
