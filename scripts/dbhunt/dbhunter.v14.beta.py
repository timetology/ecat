...
# .13	18May2020 - Added support for custom FROM statement to replace 
...
#	                --customfrom "[linkedServer01].ECAT$Secondary.dbo.winTrackingEvents_P0" or "dbo.winTrackingEvents_P1"
#                   also removed tqdm support temporarily
# .14   19May2020 - Added Support for Linked Servers using OPENQUERY([linkedserver],'query'), 
#                   Also added support to create output dirs if they not exist
# TODO
...
def Run_SQL_File(db,filename,customfrom):
	global g_debug
	try:
		txt = open(filename, 'rb')
	except IOError:
		...
		if customfrom:
			...
			tempfile = re.sub('\[dbo\]\.\[WinTrackingEventsCache\])',customfrom,tempfile)
			tempfile = re.sub('\[dbo\]\.\[WinTrackingEvents_P0\]', customfrom,tempfile)
			tempfile = re.sub('\[dbo\]\.\[WinTrackingEvents_P1\]',customfrom,tempfile)
...
def Run_SQL_NoHeaders(db,query):
	if g_debug:
		#print('[+] Processing SQL')
		start_time = timeit.default_timer()
...
def main():
	...
	parser.add_argument('-l','--linkedservers', help='Query linked servers', action='store_true', default=False)
	...
	if args.linkedservers:
		...
		if g_debug:
			print "[+] Retrieving LinkedServer List"
		txt = "SELECT LinkedServerName from uvw_SecondaryLinkedServers"
		try:
			LinkedServerList = Run_SQL_NoHeaders(db,txt)
		except Exception as e:
			print(' [!] Error: {}'.format(e))
			exit()
		if len(LinkedServerList) <= 0:
			raise Exception("[!] Error: No Linked Servers returned")
			exit()
		if g_debug:
			print LinkedServerList
		#exit()
	...
	for file in filelist:
		...
		#Handle Custom FROM replacement
		if args.customfrom:
			txt = re.sub('\[dbo\]\.\[WinTrackingEventsCache\] AS \[se\] WITH\(NOLOCK\)',args.customfrom,txt)
		...
		if args.linkedservers:
			for ls in LinkedServerList:
				...
				#print(ls[0])
				if g_debug:
					print('[?] Processing {}'.format(ls[0]))
				txt = txt.replace("'","''")
				txt = "SELECT * FROM OPENQUERY([" + ls[0] +"],'" + txt + "')"
				try:
					output = output + Run_SQL(db,txt)
					#output = Run_SQL_File(db,file,args.customfrom)
				except Exception as e:
					if g_debug:
						if '[-]' not in str(e.args):
							print(' [-] Error: {}'.format(e))
